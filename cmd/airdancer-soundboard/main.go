...
// CLI represents the command line interface for airdancer-soundboard
type CLI struct {
...
func NewCLI(cfg *soundboard.Config, stdout, stderr io.Writer) *CLI {
	return &CLI{
		config: cfg,
		stdout: stdout,
		stderr: stderr,
...
// cmdStart starts the soundboard server
func (c *CLI) cmdStart(cfg *soundboard.Config) error {
	srv, err := soundboard.NewServer(cfg)
	if err != nil {
